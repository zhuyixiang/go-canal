...
func (s *clientTestSuite) TearDownSuite(c *check.C) {
	if s.c == nil {
		return
	}
...
func (s *clientTestSuite) testConn_DropTable(c *check.C) {
	_, err := s.c.Execute("drop table if exists mixer_test_conn")
	c.Assert(err, check.IsNil)
}

func (s *clientTestSuite) testConn_CreateTable(c *check.C) {
...
func (s *clientTestSuite) TestConn_Ping(c *check.C) {
	err := s.c.Ping()
	c.Assert(err, check.IsNil)
}

func (s *clientTestSuite) TestConn_TLS(c *check.C) {
...
func (s *clientTestSuite) TestConn_Insert(c *check.C) {
	str := `insert into mixer_test_conn (id, str, f, e) values(1, "a", 3.14, "test1")`

	pkg, err := s.c.Execute(str)
	c.Assert(err, check.IsNil)
	c.Assert(pkg.AffectedRows, check.Equals, uint64(1))
}

func (s *clientTestSuite) TestConn_Select(c *check.C) {
	str := `select str, f, e from mixer_test_conn where id = 1`

	result, err := s.c.Execute(str)
	c.Assert(err, check.IsNil)
	c.Assert(result.Fields, check.HasLen, 3)
	c.Assert(result.Values, check.HasLen, 1)

	ss, _ := result.GetString(0, 0)
	c.Assert(ss, check.Equals, "a")

	f, _ := result.GetFloat(0, 1)
	c.Assert(f, check.Equals, float64(3.14))

	e, _ := result.GetString(0, 2)
	c.Assert(e, check.Equals, "test1")
...
func (s *clientTestSuite) TestConn_Escape(c *check.C) {
	e := `""''\abc`
	...
	c.Assert(ss, check.Equals, e)
}

func (s *clientTestSuite) TestConn_SetCharset(c *check.C) {
...
func (s *clientTestSuite) testStmt_DropTable(c *check.C) {
...
func (s *clientTestSuite) testStmt_CreateTable(c *check.C) {
...
func (s *clientTestSuite) TestStmt_Delete(c *check.C) {
...
func (s *clientTestSuite) TestStmt_Insert(c *check.C) {
	...
	c.Assert(r.AffectedRows, check.Equals, uint64(1))
}

func (s *clientTestSuite) TestStmt_Select(c *check.C) {
...
func (s *clientTestSuite) TestStmt_NULL(c *check.C) {
	...
	c.Assert(result.AffectedRows, check.Equals, uint64(1))
...
func (s *clientTestSuite) TestStmt_Unsigned(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	c.Assert(u, check.Equals, uint64(255))
}

func (s *clientTestSuite) TestStmt_Signed(c *check.C) {
...
func (s *clientTestSuite) TestStmt_Trans(c *check.C) {
